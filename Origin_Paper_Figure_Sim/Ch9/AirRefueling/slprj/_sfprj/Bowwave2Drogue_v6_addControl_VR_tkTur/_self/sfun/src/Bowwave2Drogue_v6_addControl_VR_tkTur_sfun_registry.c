...
{
  extern unsigned int
    sf_Bowwave2Drogue_v6_addControl_VR_tkTur_process_check_sum_call( int nlhs,
    mxArray * plhs[], int nrhs, const mxArray * prhs[] );
  if (sf_Bowwave2Drogue_v6_addControl_VR_tkTur_process_check_sum_call(nlhs,plhs,
       nrhs,prhs))
    return 1;
  return 0;
}
...
{
  extern unsigned int
    sf_Bowwave2Drogue_v6_addControl_VR_tkTur_autoinheritance_info( int nlhs,
    mxArray * plhs[], int nrhs, const mxArray * prhs[] );
  char commandName[64];
  ...
  if (strcmp(machineName, "Bowwave2Drogue_v6_addControl_VR_tkTur") == 0) {
    const mxArray *newRhs[3] = { NULL, NULL, NULL };

    newRhs[0] = prhs[0];
    newRhs[1] = prhs[2];
    newRhs[2] = prhs[3];
    return sf_Bowwave2Drogue_v6_addControl_VR_tkTur_autoinheritance_info(nlhs,
      plhs,3,newRhs);
  }
...
{
  extern unsigned int
    sf_Bowwave2Drogue_v6_addControl_VR_tkTur_third_party_uses_info( int nlhs,
    mxArray * plhs[], int nrhs, const mxArray * prhs[] );
  char commandName[64];
  char machineName[128];
  if (nrhs < 4) {
  ...
  if (strcmp(machineName, "Bowwave2Drogue_v6_addControl_VR_tkTur") == 0) {
    ...
    newRhs[1] = prhs[2];
    newRhs[2] = prhs[3];
    return sf_Bowwave2Drogue_v6_addControl_VR_tkTur_third_party_uses_info(nlhs,
      plhs,3,newRhs);
  }
...
{
  extern unsigned int
    sf_Bowwave2Drogue_v6_addControl_VR_tkTur_get_eml_resolved_functions_info
...
extern unsigned int sf_Bowwave2Drogue_v6_addControl_VR_tkTur_method_dispatcher
...
unsigned int sf_machine_global_method_dispatcher(SimStruct *simstructPtr, const
  ...
  method, void *data)
{
  if (!strcmp(machineName,"Bowwave2Drogue_v6_addControl_VR_tkTur")) {
    return(sf_Bowwave2Drogue_v6_addControl_VR_tkTur_method_dispatcher
...
extern void Bowwave2Drogue_v6_addControl_VR_tkTur_terminator(void);
void sf_machine_global_terminator(void)
{
  sfGlobalMdlStartCallCounts--;
  if (sfGlobalMdlStartCallCounts == 0) {
    Bowwave2Drogue_v6_addControl_VR_tkTur_terminator();
...
extern void Bowwave2Drogue_v6_addControl_VR_tkTur_initializer(void);
extern void Bowwave2Drogue_v6_addControl_VR_tkTur_register_exported_symbols
  (SimStruct* S);
extern void Bowwave2Drogue_v6_addControl_VR_tkTur_debug_initialize(struct
  SfDebugInstanceStruct*);
void sf_register_machine_exported_symbols(SimStruct* S)
{
  Bowwave2Drogue_v6_addControl_VR_tkTur_register_exported_symbols(S);
}
...
void sf_machine_global_initializer(SimStruct* S)
{
  ...
  if (sfGlobalMdlStartCallCounts == 1) {
    ...
    if (!simModeIsRTWGen) {
      Bowwave2Drogue_v6_addControl_VR_tkTur_debug_initialize
        (sfGlobalDebugInstanceStruct);
    }

    Bowwave2Drogue_v6_addControl_VR_tkTur_initializer();
  }
