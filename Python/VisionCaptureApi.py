...
class imuDataCopter:
    ...
    def __init__(self):
        ...
        if isEnableRosTrans:
            ...
            self.imu_pub = rospy.Publisher("/rflysim/imu",sensor.Imu,queue_size=1)
    ...
    def Imu2ros(self):
        if isEnableRosTrans:
            ...
            if(self.isUseTimeAlign):
                ...
                ros_imu.header.stamp = self.rostime + rospy.Duration(imu[0] - self.time_record)
                acc[0] = imu[1][0][0]
                acc[1] = imu[1][0][1]
                acc[2] = imu[1][0][2]
                rate[0] = imu[1][1][0]
            ...
            ros_imu.angular_velocity.x = rate[0]
            ros_imu.angular_velocity.y = rate[1]
            ros_imu.angular_velocity.z = rate[2]
            ros_imu.angular_velocity_covariance[0] = 0.01
            ros_imu.angular_velocity_covariance[4] = 0.01
            ros_imu.angular_velocity_covariance[8] = 0.01
            ros_imu.linear_acceleration_covariance[0] = 0.01
            ros_imu.linear_acceleration_covariance[4] = 0.01
            ros_imu.linear_acceleration_covariance[8] = 0.01
            ros_imu.orientation_covariance[0] = -1.0
                # ros_imu.header.stamp.secs = self.timestmp
...
class VisionCaptureApi:
    ...
    def sendUE4Pos(self,copterID=1,vehicleType=3,MotorRPMSMean=0,PosE=[0,0,0],AngEuler=[0,0,0],windowID=-1):
        ...
        """
        VelE=[0,0,0]
        self.sendUE4PosSimple(copterID,vehicleType,PosE,AngEuler,VelE,[MotorRPMSMean]*8,-1,windowID)
    ...
    def sendUE4PosSimple(self,copterID=1,vehicleType=3,PosE=[0,0,0],AngEuler=[0,0,0],VelE=[0,0,0],PWMs=[0]*8,runnedTime=-1,windowID=-1):
        """ send the position & angle information to RflySim3D to create a new 3D model or update the old model's states
            # //输出到模拟器的数据
            ...
            if len(seqList)==recPackNum: #如果收到的包达到总数了，开始处理图像
                ...
                if dataOk: #如果数据都没问题，开始处理图像
                    ...
                    if self.VisSensor[idx].SendProtocol[0]==1 or self.VisSensor[idx].SendProtocol[0]==3:
                        ...
                        if self.VisSensor[idx].SendProtocol[0]==1 and (self.VisSensor[idx].TypeID==4 or self.VisSensor[idx].TypeID==5):
                            # print('')
                            posAng = np.frombuffer(data_total,dtype = np.float32,count=6) # pos ang
                            PointNum = np.frombuffer(data_total,dtype = np.int32,count=1,offset=4*6) # num
                            PointNum = PointNum[0]
                            #print('PointNum: ',PointNum)
                            #print('posAng: ', posAng)
                            self.ImgData[idx] = posAng.tolist() + [PointNum]
                            
                            L=np.frombuffer(data_total,dtype = np.int16,count=PointNum*3,offset=4*7) # cloud
                            self.Img[idx] = L.reshape(PointNum, 3) #reshape array to 4 channel image array H X W X 4
                            self.Img[idx] = self.Img[idx] /32767.0 * self.VisSensor[idx].otherParams[0]
                            self.hasData[idx] = True
                            self.timeStmp[idx]=timeStmpStore
                    ...
                    if isEnableRosTrans and self.hasData[idx]: #如果需要发布ros消息
                        ...
                        topic_name = "/sensor"+str(self.VisSensor[idx].SeqID)
                        frame_id = "sensor"+str(self.VisSensor[idx].SeqID)
                        ...
                        if(self.time_record[idx] < 0.0000001):
                            ...
                            continue;
                        header.stamp = self.rostime[idx] + rospy.Duration(timeStmpStore-self.time_record[idx])
                        type_id = self.VisSensor[idx].TypeID
                        type = Any
                        msg = Any
                        if(type_id == 1 or type_id == 2 or type_id ==3 or type_id == 6):
                            encoding_ = "bgr8"
                            type = sensor.Image
                            msg = sensor.Image()
                            msg.header = header
            ...
            for kk in range(len(idxList)):
                ...
                for ii in range(3): #尝试读取三次内存区域
                    ...
                    if(flag[0]==2): #图像已写入完成
                        ...
                        self.timeStmp[idx] = np.frombuffer(mm,dtype = np.float64,count=1,offset=1)#struct.unpack('d',L[1:9]) #获得时间戳
                        #print('#1:',self.timeStmp[idx])
                        ...
                        if typeID==1 or typeID==2 or typeID==3:
                            ...
                            posAng = np.frombuffer(mm,dtype = np.float32,count=6,offset=9) # pos ang
                            PointNum = np.frombuffer(mm,dtype = np.int32,count=1,offset=9+4*6) # num
                            PointNum = PointNum[0]
                            #print('PointNum: ',PointNum)
                            #print('posAng: ', posAng)
                            self.ImgData[idx] = posAng.tolist() + [PointNum]
                            
                            L=np.frombuffer(mm,dtype = dtyp,count=PointNum*dim,offset=9+4*7) # cloud
                            self.Img[idx] = L.reshape(PointNum, dim) #reshape array to 4 channel image array H X W X 4
                            self.Img[idx] = self.Img[idx] /32767.0 * self.VisSensor[idx].otherParams[0]
                            if self.isRemoteSend:
                                L=np.frombuffer(mm,dtype = np.uint8,count=PointNum*dim*2+4*7,offset=9) # pos ang num cloud
