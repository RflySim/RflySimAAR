...
//!!从这里开始是系统代码的主体!!//
//这里xu是系统的输入，就是说Simulink模块的输入，18个//
//而xdot是系统的输出，就是说Simulink模块的输出，18个//
//输入输出相同只是巧合，没有对应关系//
void nlplant1(double *xu, double *xdot){

  int fi_flag;

  /* #include f16_constants */
  double g    = 32.17;          /* gravity, ft/s^2 */
  double m    = 636.94;         /* mass, slugs or 9295.6801 kg */
...
/*NasaData        %translated via eq. 2.4-6 on pg 80 of Stevens and Lewis*/
  ...
  double Jy  = 55814.0;           /* slug-ft^2 or 75674.12627 kg-m^2 */ 
  double Jxz = 982.0;             /* slug-ft^2 or 1331.42208 kg-m^2 */     
  double Jz  = 63100.0;           /* slug-ft^2 or 85552.68154 kg-m^2 */
  double Jx  = 9496.0;            /* slug-ft^2 or 12874.93287 kg-m^2*/
  ...
  double U1,V1,W1,vt1,alpha1,beta1;
  ...
  P     = xu[9]+xu[21];     /* Roll Rate --- rolling  moment is Lbar */
  Q     = xu[10]+xu[22];    /* Pitch Rate--- pitching moment is M */
  R     = xu[11]+xu[23];    /* Yaw Rate  --- yawing   moment is N */
  ...
  /* %%%%%%%%%%%%%%%%%%
   ...
   U1 = vt*ca*cb+xu[18];  /* directional velocities. */
   V1 = vt*sb+xu[19];
   W1 = vt*sa*cb+xu[20];
   vt1=sqrt(U1*U1+V1*V1+W1*W1);
   alpha1=atan(W1/vt1)*r2d;
   beta1=asin(V1/vt1)*r2d;
  
atmos(alt,vt1,temp);
   mach = temp[0];
   qbar = temp[1];  //free-stream dynamic pressure
   ps   = temp[2];  //static pressure
...
if (fi_flag == 1)          /* HIFI Table */
{
    ...
    hifi_rudder(alpha1,beta1,temp);
        delta_Cy_r30 = temp[0];
        delta_Cn_r30 = temp[1];
        delta_Cl_r30 = temp[2];

    hifi_ailerons(alpha1,beta1,temp);
        delta_Cy_a20     = temp[0];
    ...
    hifi_other_coeffs(alpha1,el,temp);
        delta_Cnbeta = temp[0];
        delta_Clbeta = temp[1];
        delta_Cm     = temp[2];
        eta_el       = temp[3];
...
/* The lofi model does not include the
   ...
   be sure we will set it to zero. */
    ...
    Cy = -.02*beta1 + .021*dail + .086*drud;

    cz(alpha1,beta1,el,temp);
        Cz = temp[0];
/*##################################################
...
//    U = vt*ca*cb+xu[18];  /* directional velocities. */
//    V = vt*sb+xu[19];
//    W = vt*sa*cb+xu[20];
//    vt=sqrt(U*U+V*V+W*W);
