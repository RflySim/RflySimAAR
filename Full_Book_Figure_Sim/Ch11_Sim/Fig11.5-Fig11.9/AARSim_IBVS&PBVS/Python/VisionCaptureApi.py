    ...
    def Imu2ros(self, node=None):
        ...
        if isEnableRosTrans:
            ...
            if self.isUseTimeAlign:
                ...
                if is_use_ros1:
                    self.ros_imu.header.stamp = self.rostime + rospy.Duration(
                        imu[0] - self.time_record
                    )
                else:
                    rclpy_time = self.rostime + Duration(
                        seconds=imu[0] - self.time_record, nanoseconds=0
                    )
                ...
                if is_use_ros1:
                    self.ros_imu.header.stamp = self.rostime + rospy.Duration(
                        self.timestmp - self.time_record
                    )
                else:
                    rclpy_time = self.rostime + Duration(
                        seconds=self.time_queue - self.time_record, nanoseconds=0
                    )
                    self.ros_imu.header.stamp = rclpy_time.to_msg()
                acc[0] = self.acc[0]
                acc[1] = self.acc[1]
...
class VisionCaptureApi:
    ...
    def sendReqToUE4(self, windID=0, IP=""):
        ...
        for i in range(len(self.VisSensor)):
            ...
            floValue = (
                [vs.CameraFOV] + vs.SensorPosXYZ + vs.SensorAngEular + vs.otherParams
            )
            buf = struct.pack("16H15f", *intValue, *floValue)
            self.udp_socket.sendto(buf, (IP, 20010 + windID))
                ...
                if dataOk:  # 如果数据都没问题，开始处理图像
                    ...
                    if (
                        ...
                        if self.VisSensor[idx].SendProtocol[0] == 1 and (
                            self.VisSensor[idx].TypeID == 4
                            or self.VisSensor[idx].TypeID == 5
                            or self.VisSensor[idx].TypeID == 6
                        ):
                            # print('')
                    ...
                    if isEnableRosTrans and self.hasData[idx]:  # 如果需要发布ros消息
                        ...
                        if type_id == 1 or type_id == 2 or type_id == 3:
                            ...
                            if is_use_ros1:
                                self.sensor_data[seq_id].header.stamp = self.rostime[
                                    idx
                                ] + rospy.Duration(
                        ...
                        if type_id == 4 or type_id == 5 or type_id == 6:
                            if type_id == 4 and not seq_id in self.topic_name.keys():
                                self.topic_name[seq_id] = (
                                    "/rflysim/sensor" + seq_id + "/vehicle_lidar"
                                )
                            if type_id == 5 and not seq_id in self.topic_name.keys():
                                self.topic_name[seq_id] = (
                                    "/rflysim/sensor" + seq_id + "/global_lidar"
                                )
                            if type_id == 6 and not seq_id in self.topic_name.keys():
                                self.topic_name[seq_id] = (
                        ...
                        if is_use_ros1:
                            ...
                            self.sensor_data[seq_id].header.stamp = self.rostime[
                                idx
                            ] + rospy.Duration(timeStmpStore - self.time_record[idx])
                ...
                for ii in range(3):  # 尝试读取三次内存区域
                    ...
                    if flag[0] == 2:  # 图像已写入完成
                        ...
                        if isEnableRosTrans and self.hasData[idx]:  # 如果需要发布ros消息
                            ...
                            if self.time_record[idx] < 0.0000001:
                                self.time_record[idx] = self.timeStmp[idx]
                                self.rostime[idx] = rospy.Time.now()
                                continue
                            if is_use_ros1:
                                header.stamp = self.rostime[idx] + rospy.Duration(
                                    self.timeStmp[idx] - self.time_record[idx]
                                )
                            else:
                                rclpy_time = self.rostime[idx] + Duration(
                                    seconds=(
    ...
    def UE4MsgRecLoop(self):
        ...
        while True:
            ...
            # }
            while True:
                if self.stopFlagUE4 or (
                    isEnableRosTrans and ((is_use_ros1 and rospy.is_shutdown))
                ):
                    break
                    ...
                    if len(buf) == 120:
                        ...
                        if iValue[0] == 1234567897:
                            ...
                            for i in range(len(self.inSilVect)):  # 遍历数据列表，飞机ID有没有出现过
                                if (
                                    self.inSilVect[i].CopterID == iValue[1]
                                ):  # 如果出现过，就直接更新数据
                                    isCopterExist = True
                    ...
                    if len(buf) == 160:
                        ...
                        if iValue[0] == 1234567897:
                            ...
                            for i in range(len(self.inReqVect)):  # 遍历数据列表，飞机ID有没有出现过
                                if (
                                    self.inReqVect[i].copterID == iValue[1]
                                ):  # 如果出现过，就直接更新数据
                                    isCopterExist = True
